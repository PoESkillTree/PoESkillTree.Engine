        ...
        protected override IReadOnlyList<MatcherData> CreateCollection() =>
            new ConditionMatcherCollection(_modifierBuilder)
            {
                ...
                { "when you ({ActionMatchers}) an enemy", Reference.AsAction.On },
                {
                    ...
                    And(OpponentOfSelf.IsRareOrUnique, Reference.AsAction.On)
                },
                {
                    "when projectile ({ActionMatchers}) a rare or unique enemy",
                    And(OpponentOfSelf.IsRareOrUnique, Condition.WithPart(Keyword.Projectile), Reference.AsAction.On)
                },
                {
                    "(when you|on) ({ActionMatchers}) a ({AilmentMatchers}) enemy",
                    And(References[1].AsAilment.IsOn(OpponentOfSelf), References[0].AsAction.On)
                },
                {
                    "(when you|on) ({ActionMatchers}) a cursed enemy",
                    And(Buffs(targets: OpponentOfSelf).With(Keyword.Curse).Any(), References[0].AsAction.On)
                },
                // kill
                { "if you or your totems kill an enemy", Or(Kill.On, Kill.By(Entity.Totem).On) },
                { "affecting enemies you kill", Kill.On },
                // hit
                { "when hit", Hit.By(OpponentOfSelf).On },
                { "when you are hit( by an enemy)?", Hit.By(OpponentOfSelf).On },
                { "with hits", Hit.On },
                ...
                },
                {
                    ...
                    (And(ModifierSourceIs(ItemSlot.MainHand), MainHandAttack, Hit.On, Buff.Blind.IsOn(OpponentOfSelf)),
                        And(ModifierSourceIs(ItemSlot.OffHand), OffHandAttack, Hit.On, Buff.Blind.IsOn(OpponentOfSelf)))
                },
                ...
                { "when you take a critical strike", CriticalStrike.By(OpponentOfSelf).On },
                // skill cast
